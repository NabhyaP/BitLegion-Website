...
<template>
  <main style="padding:2rem;max-width:40rem;margin:0 auto">
    <nav style="font-size:0.9rem;margin-bottom:1.5rem;color:#94a3b8">
    ...
    <div v-else-if="is404" style="text-align:center;padding:3rem">
      ...
      <p style="color:#64748b">This user's profile is not available.</p>
    ...
    <template v-else-if="profile">
      ...
      <div style="display:flex;align-items:center;gap:1.5rem;margin-bottom:2rem;flex-wrap:wrap">
        <div style="width:80px;height:80px;border-radius:50%;overflow:hidden;background:#e2e8f0;flex-shrink:0">
          <img v-if="profile.avatarUrl" :src="profile.avatarUrl"
               :alt="`${profile.displayName} avatar`"
               width="80" height="80"
               style="object-fit:cover;width:100%;height:100%" />
          <span v-else style="display:flex;align-items:center;justify-content:center;height:100%;font-size:2rem;color:#94a3b8" aria-hidden="true">👤</span>
        </div>
        <div>
          <h1 style="margin:0;font-size:1.4rem;color:#1e293b">{{ profile.displayName }}</h1>
        ...
        <div style="background:#f8fafc;padding:1rem;border-radius:6px;text-align:center;border:1px solid #e2e8f0">
          <div style="font-size:1.5rem;font-weight:700;color:#1e293b">
            <template v-if="profile.solvedCount === null">
              <span title="Syncing — updates daily" style="color:#94a3b8">—</span>
            </template>
            <template v-else>{{ profile.solvedCount }}</template>
          </div>
          <div style="font-size:0.75rem;color:#64748b">Solved</div>
        </div>
      </div>

      <!-- Stale notice -->
      <div v-if="profile.stale"
           style="font-size:0.8rem;color:#d97706;background:#fef3c7;border-radius:4px;padding:0.5rem 0.75rem;margin-bottom:1rem">
        ⚠ This data is from a previous snapshot. Updated {{ snapshotAge(profile.profileUpdatedAt) }}.
      </div>
      <div v-else style="font-size:0.75rem;color:#94a3b8;margin-bottom:1rem">
