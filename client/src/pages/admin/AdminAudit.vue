...
<template>
  <div>
    ...
    <div style="display:flex;flex-wrap:wrap;gap:0.6rem;margin-bottom:1rem;align-items:flex-end">
      <div>
        <label for="a-action" style="display:block;font-size:0.75rem;color:var(--muted);margin-bottom:0.2rem">Action keyword</label>
      ...
      <div>
        <label for="a-actor" style="display:block;font-size:0.75rem;color:var(--muted);margin-bottom:0.2rem">Actor user ID</label>
        <input id="a-actor" v-model.number="filters.actor" type="number" min="1" placeholder="user ID"
               style="padding:0.35rem 0.6rem;border:1px solid var(--line);border-radius:4px;font-size:0.85rem;width:100px"
               @change="filters.page=1;load()" />
      </div>
      <button style="padding:0.35rem 0.8rem;border:1px solid var(--line);border-radius:4px;cursor:pointer;font-size:0.85rem"
    ...
    <div v-if="loading && !events.length" role="status" style="padding:2rem;text-align:center;color:var(--muted)">Loading…</div>
    <div v-else-if="!events.length && !loading" style="padding:2rem;text-align:center;color:var(--muted)">No audit events found.</div>
          ...
          <template v-for="e in events" :key="e.id">
            <tr style="border-bottom:1px solid var(--surface);cursor:pointer" @click="toggleExpand(e.id)">
              ...
              <td style="padding:0.5rem;color:var(--muted);font-size:0.75rem">
                {{ e.targetType ?? '' }}<span v-if="e.targetId" style="color:var(--muted)"> #{{ e.targetId }}</span>
              </td>
              <td style="padding:0.5rem;color:var(--muted);font-size:0.7rem;max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                {{ shortJson(e.beforeJson) }}
              </td>
              <td style="padding:0.5rem;color:var(--muted);font-size:0.7rem;max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                {{ shortJson(e.afterJson) }}
              </td>
              <td style="padding:0.5rem;font-size:0.65rem;color:var(--muted);font-family:monospace">
                {{ e.requestId ? e.requestId.slice(0, 10) + '…' : '—' }}
              </td>
            </tr>
            <!-- Expanded row -->
            <tr v-if="expanded===e.id" style="background:var(--surface)">
              <td colspan="7" style="padding:0.75rem 1rem;font-size:0.8rem">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
                  <div>
                    <div style="font-weight:600;margin-bottom:0.25rem;color:var(--muted)">Before</div>
                    <pre style="margin:0;background:var(--surface);border:1px solid var(--line);border-radius:4px;padding:0.5rem;font-size:0.75rem;overflow-x:auto;white-space:pre-wrap">{{ JSON.stringify(e.beforeJson, null, 2) }}</pre>
                  </div>
                  <div>
                    <div style="font-weight:600;margin-bottom:0.25rem;color:var(--muted)">After</div>
                    <pre style="margin:0;background:var(--surface);border:1px solid var(--line);border-radius:4px;padding:0.5rem;font-size:0.75rem;overflow-x:auto;white-space:pre-wrap">{{ JSON.stringify(e.afterJson, null, 2) }}</pre>
                  </div>
                </div>
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-top:1rem;font-size:0.8rem;color:var(--muted)">
      ...
      <div style="display:flex;gap:0.5rem">
        <button :disabled="filters.page <= 1"
                style="padding:0.25rem 0.6rem;border:1px solid var(--line);border-radius:4px;cursor:pointer"
